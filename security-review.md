...
日期：2026-09-14。范围：Codebot 聊天提示组装、OpenCode 请求路由、Codex thread 恢复、聊天日志读写。未做全产品渗透测试、真实模型提示注入攻击或真实账单对比。

本次补充：Codex 选定 Skill 此前同时注入 developer 正文（每份最多 12,000 字符）和原生 skill input，现以本轮实际原生路径去重，保留 OpenCode 和缺失路径回退。移除 OpenCode/Codex/Rakazo 的 internal_prompt 事件生产；此前主聊天消费端已拦截该事件，本次为减少隐私暴露面，并非发现已发生前端外泄。回归通过模拟 Codex 运行入口验证原生 Skill 保留、正文不重复及事件流只返回任务结果；不等同真实上游账单或对抗攻击验收。
...
- OpenCode 主聊天把 system 与用户 parts 分开；Codex 使用 developerInstructions；执行器不再生成 internal_prompt 事件，聊天流聚合处仍丢弃旧适配器同类事件。不能把角色分离称为彻底防止模型复述系统提示。
