...
应用程序配置文件（设置 → 通用设置 → 配置文件 → `config.json`）里的 `server_url` 需要与 `opencode serve` 保持一致（端口/地址一致）。如果连不上，请优先检查该配置项。
...
如果 OpenCode Server 未启动，应用将无法连接到 OpenCode。请先确保 `opencode serve` 正常运行，并检查 `config.json` 中的 `server_url` 与其保持一致。
...
- **自动提取**: 每次对话后，后台自动从用户消息中识别并保存习惯/偏好/个人信息，无需手动说"帮我记住"
...
- **页面入口**: /memory/active、/memory/archived、/memory/backup、/memory/config
