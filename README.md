...
- **Hermes 共享修复**: 当用户在聊天里显式选中 Hermes skill 时，Codebot 现在会把 Hermes `skills.external_dirs` 从“全量共享根目录”收窄为“所选 skill 的精确目录集合”；未显式选中 skill 时，仍保持默认全量共享。这样可以避免 Hermes 在调用单个 skill 时继续扫描整包共享 roots，减少“CLI 已启动但长时间静默”的情况
