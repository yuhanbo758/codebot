...
Codex 会话支持复用 OpenCode 已连接的 Chat Completions、Anthropic 和 Responses 模型，也支持通过后端桥刷新 OpenCode 内置 OpenAI OAuth 授权。未知协议或尚未支持的 OAuth 插件仍会明确标为不可代理。模型目录暂不可用时保留已选模型，不自动切换到 Codex 原生模型。
...
OpenCode 对话绑定项目目录后，创建会话、发送消息、重试、读取回复及停止操作均使用同一项目目录，支持含中文和空格的路径；未绑定目录的对话继续使用 OpenCode 默认工作目录。
...
发布版本通过 GitHub Actions 在每次推送到 `main` 后自动递增并发布到 Releases。发布前会审计 Python、前端和 Electron 运行依赖；ChromaDB 当前无修复版本的两个服务端漏洞仅在源码检查确认项目仍只使用进程内 `PersistentClient` 时精确豁免，引入 Chroma HTTP 服务面会立即阻断发布。Windows 后端打包只收集 ONNX Runtime 核心推理入口与原生库，不包含未使用的量化、转换器和工具子包。
...
- **线程与权限**: 同一对话和项目目录复用持久化 Codex thread；项目切换时新建 thread。切换模型服务商时会核对 App Server 实际连接；若仍使用旧服务商，则卸载并恢复同一线程，保留历史，避免新模型被发往旧服务商。`plan` 使用只读沙箱，`build/editor/agent` 默认使用工作区写沙箱，需要升级权限时在聊天中审批。可在“设置 → Codex → 完全访问”打开开关并点击“保存 Codex 设置”：从下一条消息起允许完整文件系统、命令执行和联网，命令、文件修改及 MCP 工具权限请求自动允许；普通问题和 MCP 表单仍需回答。开关默认关闭，关闭并保存后下一条消息恢复原审批策略；计划模式和定时任务保留原限制，已开始的轮次不变。停止会调用 Codex turn interrupt。撤销会先恢复 Codebot Git 快照，再回滚 Codex 隐藏上下文；回滚失败则丢弃 thread 映射并从仍保留的聊天记录安全重建
...
- **直接复用模型**: 可安全代理的 OpenCode 已启用模型可直接在 Rakazo 中选择，首次创建、交接和换模均不再发送前置探测请求，历史探测失败不会阻止聊天。设置页提供模型名和 Provider 搜索、状态筛选与滚动列表，不再分页。当前连接、凭据和协议检查仍保留，实际调用错误正常反馈，不静默回退模型。
