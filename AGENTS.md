...
| 日期 | 变更内容 | 影响范围 |
|------|----------|---------|
| 2026-09-11 | 修复 Release 流水线连续依赖门禁故障：确认 Codebot 仅使用进程内 Chroma `PersistentClient`、未启动或连接其 HTTP 服务后，对当前无修复版本的 `PYSEC-2026-3813` 与 `PYSEC-2026-3814` 做精确豁免，并新增源码边界门禁；Electron 锁文件中的 `@xmldom/xmldom`、`fast-uri`、`js-yaml` 高危漏洞则升级到修复版本，不做豁免。CI 从 Node 20 升级到 Electron 43 要求的 Node 22，`setup-python` 升到当前 v7；本地 Python 审计为其余漏洞 0 项、Electron `npm audit` 为 0，Electron 24 项测试通过 | .github/workflows/release.yml, scripts/verify_chromadb_security_exception.py, electron/package-lock.json, README.md, security-review.md, AGENTS.md |
