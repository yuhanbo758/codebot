...
- CI 使用 Node 20 和 Python 3.11，在 Windows、macOS、Linux 三个平台分别构建前端、PyInstaller 后端和 Electron 安装包，并统一发布到 GitHub Releases。
...
| 日期 | 变更内容 | 影响范围 |
|------|----------|---------|
| 2026-09-11 | 修复 Release 流水线被新披露且暂无上游修复版本的 ChromaDB 服务端漏洞公告阻断：确认 Codebot 仅使用进程内 `PersistentClient`、未启动或连接 Chroma HTTP 服务后，对 `PYSEC-2026-3813` 与 `PYSEC-2026-3814` 做精确豁免；新增源码门禁，一旦引入 Chroma `HttpClient`、服务端命令或 v2 HTTP API 路径即停止发布并要求重审，其他 Python/Node 漏洞继续失败关闭。本地 `pip-audit` 复验为其余漏洞 0 项、4 条公告别名记录被忽略 | .github/workflows/release.yml, scripts/verify_chromadb_security_exception.py, README.md, security-review.md, AGENTS.md |
