...
| 日期 | 变更内容 | 影响范围 |
|------|----------|---------|
| 2026-09-13 | 修复 OpenCode 项目对话发送后无回复：创建 session 已传 directory，但 prompt_async、消息回读及 abort 漏传，导致请求进入默认项目。现将目录贯穿流式/非流式发送、重建重试、回复回读及停止链路，保留无项目默认行为；同时过滤助手事件前到达的用户文本，防止把用户原文误当作回复。新增中文及空格目录、重试、回读、停止和默认目录回归测试。 | backend/core/opencode_ws.py, backend/api/routes/chat.py, backend/tests/test_opencode_workspace.py, README.md |
...
| 2026-09-12 | 修复用户实测 Rakazo 一键启动失败 `dependency api failed to start ... unhealthy`：api 容器 `prisma migrate deploy` 报 P1000，根因是 Postgres 只在 pgdata volume 首次初始化时写入密码，旧 volume（2026-08-31 部署）保留旧密码，而 DPAPI 密钥存储（8-31 后期引入/重建）给出新密码导致认证失配。实机已通过 postgres 容器本地 trust socket `ALTER USER` 把数据库密码对齐到当前受管密钥（保留全部数据），api 恢复 healthy、全部 11 个服务就绪、`/api/rakazo/status` 返回 `connected=true`；同时在 `control_runtime` 启动链中新增 P1000 检测与受管密码自动对齐自愈（仅受管 Compose、密钥无引号、postgres 容器在运行时执行，失败关闭不变，只重试一次），新增 4 项回归测试，Rakazo 52 项测试全部通过 | backend/core/rakazo_runtime.py, backend/tests/test_rakazo_integration.py, AGENTS.md |
| 2026-09-12 | 修复 v5.5.0 安装包两个用户实测故障：① GitHub Windows runner 默认 `core.autocrlf=true` 检出把随包 Rakazo Compose 转成 CRLF，固定 SHA256 摘要校验必然失败且无法安装实验运行时；新增 `.gitattributes` 强制 LF 检出，并把 `_sha256_file` 改为 CRLF→LF 归一化后计算，双保险修复已装包与未来构建，新增 CRLF 容忍回归测试（Rakazo 48 项测试通过）。② 用户 `~/.codex/config.toml` 在 `[features]` 内写入了点号键 `context_management.experimental_mode`，TOML 会生成子表而 Codex features 只接受布尔值，导致 App Server 报 `invalid type: map, expected a boolean`（JSON-RPC -32600）无法发送消息；已修复本机配置并重启 Codex App Server 验证 96 个模型正常加载 | .gitattributes, backend/core/rakazo_runtime.py, backend/tests/test_rakazo_integration.py, AGENTS.md |
